...
kubectl exec -n hlf-production-network $CLI1 -i -- sh < scripts/startNetwork/setupChannel/createChannel.sh
...
kubectl exec -n hlf-production-network $CLI1 -i -- sh < scripts/startNetwork/setupChannel/joinChannelOrg1.sh

sep

echo "Joining channel using CLI2 on Org2 Peer1"
CLI2=$(get_pods "cli-org2")
kubectl exec -n hlf-production-network $CLI2 -i -- sh < scripts/startNetwork/setupChannel/joinChannelOrg2.sh